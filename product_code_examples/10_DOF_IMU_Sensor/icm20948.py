...
class ICM20948(object):
  ...
  def _get_pitch_roll_yaw(self):
    ...
    self.imuAHRSupdate(MotionVal[0] * 0.0175, MotionVal[1] * 0.0175,MotionVal[2] * 0.0175,
      MotionVal[3],MotionVal[4],MotionVal[5], MotionVal[6], MotionVal[7], MotionVal[8])
    returnList = [0, 0, 0]
    #pitch
    returnList[0] = math.asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3
    #roll
    returnList[1] = math.atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3
    #yaw
    returnList[2] = math.atan2(-2 * q1 * q2 - 2 * q0 * q3, 2 * q2 * q2 + 2 * q3 * q3 - 1) * 57.3 
    return returnList
