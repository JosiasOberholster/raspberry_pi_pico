"""
Product code to fetch the Tempreatur (C)
as well as the Pressure (hPA)
"""
import time
...
class LPS22HB(object):
    ...
    def __init__(self,address=LPS22HB_I2C_ADDRESS):
        ...
        self._write_byte(self.LPS_CTRL_REG1 ,0x02)   #Low-pass filter disabled , output registers
    ...
    """
    Get the current temperatur in Celsius
    ...
    def _get_temp(self):
        ...
        if (self._read_byte(self.LPS_STATUS)&0x02)==0x02:   # a new temperature data is generated
    ...
    def _get_pressure(self):
        ...
        u8Buf=[0,0,0]
        self.LPS22HB_START_ONESHOT()
        if (self._read_byte(self.LPS_STATUS)&0x01)==0x01:  # a new pressure data is generated
            ...
            PRESS_DATA_RAW=((u8Buf[2]<<16)+(u8Buf[1]<<8)+u8Buf[0])/4096.0
            PRESS_DATA = round(PRESS_DATA_RAW, 2)
        return PRESS_DATA    
